<template>
  <div class="exception-title">
    <el-button type="primary" @click="applyHandle">异常处理</el-button>
    <el-space>
  ...
  <el-row :gutter="20">
    <el-col :span="12">
      <el-empty v-if="detailMonth.length === 0" description="暂无申请审批" />
...
const ret = (signsInfos.value.detail as { [index: string]: unknown })[toZero(month.value)] as {
...
const renderTime = (date: string) => {
  const ret = (
    (signsInfos.value.time as { [index: string]: unknown })[toZero(month.value)] as {
      [index: string]: unknown
    }
  )[date]
