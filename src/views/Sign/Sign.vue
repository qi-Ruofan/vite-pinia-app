<template>
  <el-descriptions border direction="vertical" :column="9">
    ...
    <el-descriptions-item label="操作">
      <el-button type="primary" plain size="small" @click="detailClick">查看详情</el-button>
    </el-descriptions-item>
    <el-descriptions-item label="考勤状态">
      <el-tag :type="detailState.type" plain size="small">{{ detailState.text }}</el-tag>
    </el-descriptions-item>
  </el-descriptions>
  <el-calendar v-model="date">
    <template #header>
      <el-button type="primary" @click="putTimeHandle">在线签到</el-button>
      <el-space>
...
const renderTime = (day: string) => {
  ...
  const ret = (
    (signsInfos.value.time as { [index: string]: unknown })[month] as { [index: string]: unknown }
  )[date]
